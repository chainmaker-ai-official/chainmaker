...
const BollingerBands: React.FC<any> = ({ 
  ...
  const fields: SettingField[] = [
    ...
    { id: 'stdDev', label: 'Std Dev Multiplier', type: 'number' },
    { 
      id: 'source', 
      label: 'Source', 
      type: 'select', 
  ...
  return (
    <>
      ...
      >
        ...
        {isPaletteItem && (
          <div className="absolute -bottom-[1px] left-0 w-full flex justify-around">
        ...
        <div className="flex items-center gap-2 mb-2 border-b border-[#C678DD33] pb-1">
          <Layers size={14} />
          <span className="text-[10px] font-bold uppercase tracking-wider">Bollinger Bands</span>
        </div>
        ...
        <div className="flex flex-col gap-1">
          ...
          <div className="flex justify-between text-[10px]">
            ...
            <span className="text-[#61AFEF]">{settings.stdDev}</span>
          </div>
        </div>
      </div>
      ...
      {!isPaletteItem && (
        <NodeSettingsModal 
          ...
          onSave={(data) => {
            setSettings(data);
            onLabelChange?.(id, `BB(${data.period}, ${data.stdDev})`);
          }}
