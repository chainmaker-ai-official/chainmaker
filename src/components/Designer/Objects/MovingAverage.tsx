...
const MovingAverage: React.FC<any> = ({ 
    ...
    <>
      ...
      >
        ...
        <div className="flex items-center gap-2 mb-2 border-b border-[#C678DD33] pb-1">
          <TrendingUp size={14} />
          <span className="text-[10px] font-bold uppercase tracking-wider">Moving Average</span>
        </div>

        <div className="flex flex-col gap-1">
          <div className="flex justify-between text-[10px]">
            <span className="text-gray-500">Type:</span>
            <span className="text-[#61AFEF]">{settings.type}</span>
          </div>
          <div className="flex justify-between text-[10px]">
      ...
      {!isPaletteItem && (
        <NodeSettingsModal 
          ...
          onSave={(data) => {
            setSettings(data);
            onLabelChange?.(id, `${data.type}(${data.period})`);
          }}
