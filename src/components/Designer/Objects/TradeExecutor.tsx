...
const TradeExecutor: React.FC<any> = ({ 
  ...
  const fields: SettingField[] = [
    ...
    { id: 'asset', label: 'Asset Pair', type: 'text', placeholder: 'e.g., BTC/USDT' },
    ...
    <>
      ...
      >
        ...
        <div className="flex items-center gap-2 mb-2 border-b border-[#E06C7533] pb-1">
          ...
          <span className="text-[10px] font-bold uppercase tracking-wider">Trade Executor</span>
        </div>

        <div className="flex flex-col gap-1">
          <div className="flex justify-between text-[10px]">
            <span className="text-gray-500">Action:</span>
            <span className={settings.action === 'BUY' ? 'text-[#98C379]' : 'text-[#E06C75]'}>{settings.action}</span>
          </div>
          <div className="flex justify-between text-[10px]">
            <span className="text-gray-500">Asset:</span>
            <span className="text-[#61AFEF]">{settings.asset}</span>
          </div>
          <div className="flex justify-between text-[10px]">
            <span className="text-gray-500">Amount:</span>
            <span className="text-[#D19A66]">{settings.amount}</span>
          </div>
        </div>
      </div>
      ...
      {!isPaletteItem && (
        <NodeSettingsModal 
          ...
          onSave={(data) => {
            setSettings(data);
            onLabelChange?.(id, `${data.action} ${data.amount} ${data.asset}`);
          }}
