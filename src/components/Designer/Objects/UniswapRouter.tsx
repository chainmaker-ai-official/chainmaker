...
const UniswapRouter: React.FC<any> = ({ 
  ...
  inputData = null, onPriceUpdate, isSelected = false, onClick, onContextMenu,
  ...
  const fetchPrice = useCallback(async () => {
    setIsLoading(true);
    try {
      // Mock fetch for now as we don't have a real backend yet
      const mockPrice = 2500 + Math.random() * 100;
      const data = { eth_price: mockPrice, timestamp: new Date().toISOString() };
      setPriceData(data);
      if (onPriceUpdate) onPriceUpdate(id, { value: data.eth_price, raw: data, type: 'price' });
    } catch (error) {
  ...
  useEffect(() => {
    if (!isPaletteItem && inputData?.timestamp) {
      fetchPrice();
    }
  }, [inputData?.timestamp, fetchPrice, isPaletteItem]);
  ...
  return (
    ...
    >
      ...
      <div className="flex flex-col items-center">
        <span className="text-[10px] opacity-70 uppercase">Uniswap</span>
        {isLoading ? (
          <span className="text-xs italic opacity-50">Loading...</span>
        ) : priceData ? (
          <span className="text-sm font-bold">${priceData.eth_price.toFixed(decimalPlaces)}</span>
        ) : (
          <span>{label || 'Router'}</span>
        )}
      </div>
    </div>
  );
