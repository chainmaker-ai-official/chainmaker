...
/**
 * Custom hook for handling block dragging functionality
 * Now supports multi-block dragging when blocks are selected
 */
export const useBlockDrag = (blocks, updateBlockPosition, selectedBlockIds = new Set()) => {
  ...
  const startDrag = useCallback((blockId, e, workspaceRef) => {
    ...
    if (!block || !workspaceRef?.current || !e) return;
  ...
  /**
   * Handle drop from external palette
   */
